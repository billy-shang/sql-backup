...
- 镜像：https://hub.docker.com/r/billyshang/sql-backup （当前 `v1.0.31`）
...
```bash
docker pull billyshang/sql-backup:v1.0.31
docker run -d --name sql-backup --restart unless-stopped \
  -p 8788:8788 -e TZ=Asia/Shanghai -e SQL_BACKUP_DATA_DIR=/data \
  -v "$PWD/data:/data" billyshang/sql-backup:v1.0.31
```

`data/` 必须整目录持久化（SQLite 与 `secret.key` 不能拆开）。群晖等旧 Docker 请用 `v1.0.31`（linux/amd64，关闭 provenance）：

```bash
docker build --provenance=false --sbom=false --platform linux/amd64 \
  -t billyshang/sql-backup:v1.0.31 .
```

## 说明

- 群晖上传只走 File Station（5000/5001），不走 22。
- 新增/编辑连接点击空白处不会关闭。
- 手动备份后弹窗关闭，连接行显示进度。
- 顶栏用户名下拉可改密、查看帮助、退出。
