...
- 镜像：https://hub.docker.com/r/billyshang/sql-backup （当前 `v1.0.46`）
...
打开 http://127.0.0.1:8788 ，默认 `admin` / `admin@123`。大文件归档经 SQL 分块读取时按 8MB 一块，避免每 256KB 重启一次 PowerShell。

## Docker

```bash
docker pull billyshang/sql-backup:v1.0.46
docker run -d --name sql-backup --restart unless-stopped \
  -p 8788:8788 -e TZ=Asia/Shanghai -e SQL_BACKUP_DATA_DIR=/data \
  -v "$PWD/data:/data" billyshang/sql-backup:v1.0.46
```

`data/` 必须整目录持久化（SQLite 与 `secret.key` 不能拆开）。群晖等旧 Docker 请用 `v1.0.46`（linux/amd64，关闭 provenance）：

```bash
docker build --provenance=false --sbom=false --platform linux/amd64 \
  -t billyshang/sql-backup:v1.0.46 .
```
